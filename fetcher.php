<?php
	...
	class LuckyFetcher
	{
		...
		function __construct()
		{
			...
			$this->sql = mysql_connect("localhost", $msqu, $msqp);
		...
		function update($force = false)
		{
			...
			if($update_f)
			{
				...
				$res = mysql_query("SELECT * FROM `lcat_plist`", $this->sql);
				$tbl = array();
				...
				for($i=0, $lim=count($buf[0]); $i<$lim; $i++)
				{
					...
					if(!$tbl[intval($buf[1][$i])])
					{
						...
						$out = sprintf("GET /api/p/num/".$buf[1][$i]." HTTP/1.1\r\n", $id);
						...
						$cmd = "INSERT INTO `lcat_plist` SET `pnum`='".$buf[1][$i]."', `pid`='".$b[1]."', `name`='".$buf[3][$i]."', `url`='".$buf[2][$i]."', ";
